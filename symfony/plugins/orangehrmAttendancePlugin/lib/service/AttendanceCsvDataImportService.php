 ...
 *
 */
class AttendanceCsvDataImportService extends BaseService {
...
//		return $this->getCsvDataImportService()->import($file, $importType);
		return $this->getCsvRead($file, $importType);
	}

	public function getCsvRead($file, $importType) {

		$factory = new AttendanceCsvFactory();
		...
		if (($handle = fopen($file->getTempName(), "r")) !== FALSE) {
			while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
				...
				if ($row != 2) {
					...
					$result = $instance->importAttendanceCsv($temp);
					if($result) {
						$rowsImported++;
					}
				}
